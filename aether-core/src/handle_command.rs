...
pub async  fn handle_commands(command: Command, username: String, client: Client, chat: ChatPacket, state: State) -> anyhow::Result<()> {
    match command {
        Command::PearlLoad => pearl::load::handle_load(username, client, state).await,
        Command::PearlSet(x, y, z) => pearl::set::handle_set(username, client, state, x, y, z).await,
